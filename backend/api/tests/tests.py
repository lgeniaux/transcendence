...
def test_already_logged_in(client):
    ...
    response = client.post(base_url + "/login-user/", {"username": "validusername2", "password": "17ValidPassword@"})
    assert response.status_code == status.HTTP_200_OK
    token = response.data['auth_token']
    response = client.post(base_url + "/login-user/", {"username": "validusername2", "password": "17ValidPassword@"}, HTTP_AUTHORIZATION='Token ' + token)
    assert response.status_code == status.HTTP_200_OK
