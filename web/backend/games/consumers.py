...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        if self.user:
            ...
            if await self.is_player1(self.user, self.game_id) and not await self.is_finished(self.game_id):
    ...
    async def send_update_to_group(self, message):
        group_name = f'game_{self.game_id}'
        await self.channel_layer.group_send(group_name, {'type': 'game.update', 'message': message})
