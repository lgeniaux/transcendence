...
class User(AbstractUser):
    ...
    friendlist = models.ManyToManyField('self', symmetrical=False, blank=True, related_name='friends')
    blocklist = models.ManyToManyField('self', symmetrical=False, blank=True, related_name='blocked')
...
class Tournament(models.Model):
    ...
    def start_tournament(self):
        ...
        if len(participants) != self.nb_players:
            raise ValueError("Tournament participants count does not match the expected number of players.")
        ...
        for index, game in enumerate(games):
            ...
            for player in game:
                send_notification(
                    ...
                    data={'game_id': new_game.game_id, 'tournament_id': self.id, 'round_name': new_game.round_name, 'status': 'pending'}
                )
            
            # Update the tournament state
            game_data = {'game_id': new_game.game_id, 'player1': player1.username, 'player2': player2.username, 'status': new_game.status}
            self.state['quarter-finals' if self.nb_players > 4 else 'semi-finals'].append(game_data)
    ...
    def check_round_completion(self):
        ...
        if all(Game.objects.filter(game_id=game['game_id'], status='finished').exists() for game in current_round_games):
    ...
    def prepare_next_round(self, next_round):
        ...
        for game in games:
            ...
            next_round_games.append({'game_id': new_game.game_id, 'player1': player1.username, 'player2': player2.username, 'status': new_game.status})
