        ...
        if serializer.is_valid():
            ...
            if action == "block":
                ...
                if user in request_user.blocklist.all():
                    return Response(
                        {"detail": "User is already in your blocklist"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                request_user.blocklist.add(user)
                return Response(
                    {"detail": "User successfully blocked"}, status=status.HTTP_200_OK
                )
            elif action == "unblock":
                if user not in request_user.blocklist.all():
                    return Response(
                        {"detail": "User is not in your blocklist"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                request_user.blocklist.remove(user)
                return Response(
                    {"detail": "User successfully unblocked"}, status=status.HTTP_200_OK
                )
        else:
...
class AddOrDeleteFriendSerializer(serializers.Serializer):
    ...
    def validate_friend(self, request_user, friend_username):
        ...
        if friend == request_user:
            raise serializers.ValidationError(
                {"detail": "You cannot add/delete yourself"}
            )
        return friend
        ...
        if serializer.is_valid():
            ...
            if action == "add":
                if friend in request_user.blocklist.all():
                    return Response(
                        {
                            "detail": "You cannot add a user to your friendlist if you have blocked them"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                if friend in request_user.friendlist.all():
                    return Response(
                        {"detail": "User is already in your friendlist"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                request_user.friendlist.add(friend)
                return Response(
                    {"detail": "User successfully added to your friendlist"},
                    status=status.HTTP_200_OK,
                )
            elif action == "delete":
                if friend not in request_user.friendlist.all():
                    return Response(
                        {"detail": "User is not in your friendlist"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                request_user.friendlist.remove(friend)
                return Response(
                    {"detail": "User successfully removed from your friendlist"},
                    status=status.HTTP_200_OK,
                )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def validate_notification(self, notification_id, request_user):
        try:
            notification = Notification.objects.get(
                id=notification_id, recipient=request_user
            )
        ...
        if (
            notification.notification_type == "tournament-invite"
            or notification.notification_type == "game-invite"
        ):
        ...
        if serializer.is_valid():
            ...
            if notification.notification_type == "tournament-invite":
                tournament = Tournament.objects.get(
                    id=notification.data["tournament_id"]
                )
                broadcast_message = {
                ...
                if action == "accept":
                    ...
                    return Response(
                        {"detail": "Tournament invitation successfully accepted"},
                        status=status.HTTP_200_OK,
                    )
                elif action == "deny":
                    ...
                    tournament.save()
                    return Response(
                        {"detail": "Tournament invitation successfully denied"},
                        status=status.HTTP_200_OK,
                    )
                ...
                if action == "accept":
                    ...
                    game.save()
                    message = message = (
                        f"{request_user.username} has accepted the game invite"
                    )
                    data = {"game_id": game.game_id, "status": "pending"}
                    send_notification(game.player1, message, "game-start", data)
                    message = message = (
                        f"You have accepted the game invite from {game.player1.username}"
                    )
                    data = {"game_id": game.game_id, "status": "pending"}
                    send_notification(game.player2, message, "game-start", data)
                    return Response(
                        {"detail": "Game invitation successfully accepted"},
                        status=status.HTTP_200_OK,
                    )
                elif action == "deny":
                    notification.data["status"] = "denied"
                    ...
                    game.delete()
                    return Response(
                        {"detail": "Game invitation successfully denied"},
                        status=status.HTTP_200_OK,
                    )
            return Response(
                {"detail": "Action completed successfully"}, status=status.HTTP_200_OK
            )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class InvitePlayerToGameSerializer(serializers.Serializer):
    ...
    def validate_player(self, request_user, player_username):
        ...
        if player in request_user.blocklist.all():
            raise serializers.ValidationError(
                {"detail": "You cannot invite a user you have blocked"}
            )
        ...
        existing_invitation = Notification.objects.filter(
            Q(
                ...
                data__contains={"game_id": player.id},
                notification_type="game-invite",
            )
            | Q(
                recipient=player,
                data__contains={"game_id": request_user.id},
                notification_type="game-invite",
            ),
        ...
        if ongoing_game:
            raise serializers.ValidationError(
                {"detail": "There's already an ongoing game between you two"}
            )
...
class InvitePlayerToGame(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"detail": "Game invitation sent successfully"},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
