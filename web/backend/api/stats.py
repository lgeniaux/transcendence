    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            finished_games = Game.objects.filter(
                (Q(player1=fetched_user) | Q(player2=fetched_user))
                & Q(status="finished")
            )

            finished_tournaments = Tournament.objects.filter(
                participants=fetched_user, state__status="finished"
            )
            user_stats = {
                "tournaments_played": finished_tournaments.count(),
                "tournaments_won": finished_tournaments.filter(
                    state__winner=fetched_user.username
                ).count(),
                "tournament_winrate": (
                    finished_tournaments.filter(
                        state__winner=fetched_user.username
                    ).count()
                    / finished_tournaments.count()
                    * 100
                    if finished_tournaments.count() > 0
                    else 0
                ),
                "games_played": finished_games.count(),
                "games_won": finished_games.filter(winner=fetched_user).count(),
                "game_winrate": (
                    finished_games.filter(winner=fetched_user).count()
            ...
            user_stats["average_scored_value"] = (
                finished_games.annotate(
                    score=Case(
            ...
            user_stats["average_opponent_score"] = (
                finished_games.annotate(
                    opponent_score=Case(
                ...
                ).aggregate(Avg("opponent_score"))["opponent_score__avg"]
                or 0
            )
                ...
                game_history.append(
                    {
                        ...
                        "tournament_id": (
                            game.tournament.id if game.tournament else "None"
                        ),
                    }
                )
