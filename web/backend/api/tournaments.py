...
class TournamentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        tournament = Tournament.objects.create(name=name, creator=self.context['request'].user, nb_players=nb_players)
...
class InvitePlayerToTournamentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if tournament.creator != self.context['request'].user:
            raise serializers.ValidationError("You are not the creator of this tournament.")
        ...
        if tournament.participants.count() + Notification.objects.filter(data__tournament_id=data['tournament_id'], notification_type='tournament-invite', data__status='pending').count() + 1 > tournament.nb_players:
            raise serializers.ValidationError("Maximum number of invitations + participants reached.")
        
        # if the user is already a participant in the tournament
        if tournament.participants.filter(username=data['username']).exists():
            raise serializers.ValidationError("User is already a participant in the tournament.")
        
        # if there is not already a notification for this user on the same tournament
        if Notification.objects.filter(recipient__username=data['username'], data__tournament_id=tournament_id, notification_type='tournament-invite').exists():
...
class InvitePlayerToTournament(APIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = InvitePlayerToTournamentSerializer(data=request.data, context={'request': request})
        if serializer.is_valid():
            tournament_id = serializer.validated_data['tournament_id']
            try:
                tournament = Tournament.objects.get(id=tournament_id)
                invite_participant_to_tournament(tournament, serializer.validated_data['username'], request.user.username)
