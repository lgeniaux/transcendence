...
def created_users(client):
    ...
    for i in range(1, 4):
        ...
        r = client.post(base_url + "/register-user/", {"email": email, "username": username, "password": password})
        assert r.status_code == status.HTTP_201_CREATED
        r = client.post(base_url + "/login-user/", {"email": email, "password": password})
        assert r.status_code == status.HTTP_200_OK
        assert r.data['auth_token'] is not None
        users.append({"email": email, "username": username, "password": password, "token": r.data['auth_token']})
    return users
...
def users_with_games(client, created_users):
    ...
    r = client.post(base_url + "/game/invite/", {"username": user2["username"]}, HTTP_AUTHORIZATION='Token ' + user1_token)
    assert r.status_code == status.HTTP_200_OK
    r = client.get(base_url + "/get-notifications/", HTTP_AUTHORIZATION='Token ' + user2_token)
    assert r.status_code == status.HTTP_200_OK
    assert len(r.json()) == 1
    ...
    r = client.post(base_url + "/respond-to-invite/", {"notification_id": notification_id, "action": "accept"}, HTTP_AUTHORIZATION='Token ' + user2_token)
    assert r.status_code == status.HTTP_200_OK
...
def test_logout(client, created_users):
    ...
    response = client.post(base_url + "/logout-user/", HTTP_AUTHORIZATION='Token ' + created_users[0]['token'])
    assert response.status_code == status.HTTP_200_OK
...
def test_delete_user(client, created_users):
    # Test case 1: bad token
    response = client.post(base_url + "/logout-user/", HTTP_AUTHORIZATION='Token ' + created_users[0]['token'])
    response = client.post(base_url + "/me/delete/", HTTP_AUTHORIZATION=f'Token {created_users[0]["token"]}')
    assert response.status_code == status.HTTP_401_UNAUTHORIZED

    # Test case 2: good token
    response = client.post(base_url + "/me/delete/", HTTP_AUTHORIZATION=f'Token {created_users[1]["token"]}')
    assert response.status_code == status.HTTP_200_OK
...
def test_change_password(client, created_users):
    ...
    response = client.put(base_url + "/change-password/", data=json.dumps({"old_password": user1["password"], "new_password": "18ValidPassword@", "confirm_new_password": "18ValidPassword@"}), headers={"Content-Type": "application/json", "Authorization": 'Token ' + token})
...
def test_create_game(client, created_users):
    ...
    player1_token, player2_token, player3_token = user1["token"], user2["token"], user3["token"]

    # Player 1 invites Player 2
    response = client.post(base_url + "/game/invite/", {"username": user2["username"]}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK

    # Player 1 invites unknown player
    response = client.post(base_url + "/game/invite/", {"username": "unknown"}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 1 invites himself
    response = client.post(base_url + "/game/invite/", {"username": user1["username"]}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    response = client.post(base_url + "/game/invite/", {"username": user1["username"]}, HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    response = client.post(base_url + "/game/invite/", {"username": user2["username"]}, HTTP_AUTHORIZATION='Token ' + player3_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.post(base_url + "/respond-to-invite/", {"notification_id": notification_id, "action": "random"}, HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 2 refuse invite
    response = client.post(base_url + "/respond-to-invite/", {"notification_id": notification_id, "action": "deny"}, HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
...
def test_game(client, users_with_games):
    ...
    # Player 1 get-status
    response = client.get(base_url + f"/game/get-status/{game_id}/", HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    assert response.json()['game']['status'] == "waiting to start"

    # Player 2 get-status
    response = client.get(base_url + f"/game/get-status/{game_id}/", HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    assert response.json()['game']['status'] == "waiting to start"

    # Player 3 get-status
    response = client.get(base_url + f"/game/get-status/{game_id}/", HTTP_AUTHORIZATION='Token ' + users[2]["token"])
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 2 starts the game
    response = client.post(base_url + "/game/start/", {"game_id": game_id}, HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 1 starts the game
    response = client.post(base_url + "/game/start/", {"game_id": game_id}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get(base_url + f"/profile/stats/{users[0]['username']}/", HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    assert response.json()['game_stats']['games_played'] == 0
    
    # Player 1 ends the game with bad score
    response = client.post(base_url + "/game/end/", {"game_id": game_id, "score": "5_"}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    response = client.post(base_url + "/game/end/", {"game_id": game_id, "score": "-1-5"}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    response = client.post(base_url + "/game/end/", {"game_id": game_id, "score": "-5"}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 2 ends the game
    response = client.post(base_url + "/game/end/", {"game_id": game_id, "score": "5-1"}, HTTP_AUTHORIZATION='Token ' + player2_token)
    print(response.json())
    assert response.status_code == status.HTTP_400_BAD_REQUEST

    # Player 1 ends the game with good score
    response = client.post(base_url + "/game/end/", {"game_id": game_id, "score": "5-1"}, HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get(base_url + f"/profile/stats/{users[1]['username']}/", HTTP_AUTHORIZATION='Token ' + player1_token)
    print(response.json())
    assert response.status_code == status.HTTP_200_OK
    assert response.json()['game_stats']['games_played'] == 1
